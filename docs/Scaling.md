...
From a hardware perspective, **Liberdus** is designed to scale **horizontally**.  
Horizontal scaling is a method of increasing a network's capacity by **adding more nodes**.  

In contrast, **vertical scalability** increases capacity by upgrading existing nodes; adding extra **RAM, CPU, or storage** to the same machines.

Liberdus uses **horizontal scaling** to increase:
- **Throughput (TPS)**
- **Storage**
- **Bandwidth**
...
Traditional Web3 networks that use **network sharding**, **transaction sharding**, or **static state sharding** have a fixed threshold where each shard becomes “full.”  
This threshold is determined by the maximum number of nodes permitted within a shard. Once the shard reaches that limit, a new shard must be created; a process known as **stepwise scaling**.
...
In contrast, **Liberdus employs linear scaling**.  
With **linear scaling**, the network’s capacity increases **proportionally** with each new **active node**.

For example:  
If each active Liberdus node provides **2 TPS**, and there are **1,000 active nodes**, the total network capacity is **2,000 TPS**.  
Adding two new nodes increases total throughput to **2,004 TPS**, demonstrating perfectly **predictable, linear growth**.
...
In major Web2 platforms (like Amazon or Walmart), backend systems can **dynamically scale up or down** based on traffic.  
During high-demand periods (e.g., holiday sales), server capacity might increase fivefold; then automatically shrink when demand drops.  
This ensures performance remains stable while minimizing unnecessary operating costs.
...
| Concept | Scaling Type | Method | Key Benefit | Example |
|----------|---------------|---------|--------------|----------|
| **Horizontal Scaling** | Hardware-based | Add more nodes | Increases TPS, bandwidth, and storage | Each new node increases capacity |
| **Linear Scaling** | Performance-based | Proportional growth per node | Predictable and frictionless expansion | 2 TPS per node = 2000 TPS at 1000 nodes |
| **Autoscaling** | Adaptive | Dynamic node adjustment | Real-time optimization based on network load | Nodes vote to scale up/down automatically |

Liberdus’s horizontally scalable, linearly expanding, and autonomously self-adjusting network ensures **long-term decentralization**, **efficiency**, and **sustainability**, even as global usage scales to millions of users.
