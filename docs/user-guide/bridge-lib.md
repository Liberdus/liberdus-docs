...
3. Select Bridge from the menu list.
...
1. You will see a dropdown labelled Bridge Direction.
 Select one of the following:
    - From External Network to Liberdus
        Use this when depositing LIB into your Liberdus wallet.
    - From Liberdus to External Network
        Use this when withdrawing LIB to an outside network.
2. Tap Continue
...
**A) If sending LIB to Liberdus (deposit):**
You will be shown:
- The network you must send from
- A destination LIB address (your Liberdus wallet)
- Amount
- A reminder of any required minimums or fees
You then complete the transaction using your external wallet (MetaMask, etc.).

**B) If sending LIB from Liberdus (withdraw):**
...
- If an external wallet requires a specific network (Polygon, Ethereum, etc.), make sure you are connected to the correct one.
- If a network is unavailable, the bridge will not allow the transfer direction.
